      ...
      <Section>
        {posts.length === 0 ? (
          <div className="rounded-4xl border border-dashed border-sand-300 bg-sand-50 p-16 text-center">
            ...
            <p className="mt-4 text-lg font-semibold text-ink-900">No posts yet</p>
            <p className="mt-1 text-slate-500">Check back soon for career and hiring insights.</p>
          </div>
        ) : (
...
type PostCard = Awaited<ReturnType<typeof listPublishedPosts>>[number];

function BlogList({ posts }: { posts: PostCard[] }) {
      ...
      >
        <div className="relative aspect-[16/10] overflow-hidden bg-gradient-to-br from-brand-600 to-brand-800 lg:aspect-auto">
          {featured.coverImageUrl ? (
            <Image src={featured.coverImageUrl} alt="" fill unoptimized className="object-cover transition-transform duration-500 group-hover:scale-105" />
          ) : (
            <>
        ...
        <div className="flex flex-col justify-center p-8 lg:p-10">
          ...
          <div className="mt-6 flex items-center gap-3 text-sm text-slate-500">
            ...
            <span>·</span>
            <span>{formatDate((featured.publishedAt as string | null) ?? (featured.createdAt as string))}</span>
            <span>·</span>
              ...
              >
                ...
                <div className="flex flex-1 flex-col p-6">
                  ...
                  <div className="mt-auto flex items-center gap-2 pt-6 text-xs text-slate-500">
                    ...
                    <span>·</span>
                    <span>{formatDate((post.publishedAt as string | null) ?? (post.createdAt as string))}</span>
                  </div>
                </div>
              </Link>
            </MotionItem>
